...
def load_data(train_filename):
    ...
    encoder = (
        LabelEncoder()
    )  # Initialize the encoder, we just want simple integer representation for string data like TCP/UDP.
...
def train(x):
    ...
    model = Sequential(
        [
            ...
            ),  # The input shape is (x.shape[1], x.shape[2]), which corresponds to the number of time steps and features in the input data.
            layers.Conv1D(
                filters=32,
                kernel_size=7,
                padding="same",
